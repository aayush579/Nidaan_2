...
const IssueCard = ({ issue }) => (
  <div className="bg-card rounded-lg border border-border overflow-hidden hover:shadow-lg transition-shadow">
    ...
    <div className="p-6">
      ...
      <h3 className="text-lg font-semibold text-foreground mb-2 line-clamp-2">{issue.title}</h3>
      <p className="text-muted-foreground text-sm mb-4 line-clamp-3">{issue.description}</p>
      ...
      <div className="space-y-2 mb-4">
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <div className="flex items-center space-x-2">
            <img src={issue.reporterAvatar} alt={issue.reportedBy} className="w-5 h-5 rounded-full" />
            <span className="text-foreground font-medium">{issue.reportedBy}</span>
          </div>
        </div>
      </div>

      <Button variant="outline" fullWidth className="text-accent border-accent hover:bg-accent hover:text-white">
        View Details
      </Button>
    </div>
  </div>
